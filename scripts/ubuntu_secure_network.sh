...
function_validation() { # {{{
  ...
  echo "    [Check] Automatically generating DNS traffic for 5 seconds..."

  # 💡 Trigger background HTTPS request to generate encrypted DNS traffic.
  (sleep 1 && curl -sI https://google.com >/dev/null 2>&1) &

  # ✅ Terminate tcpdump automatically after 5 seconds or 10 packets.
  # ℹ️ Requires 'timeout' utility (coreutils).
  sudo timeout 5 tcpdump -ni any port 53 or port 853 -c 10
  echo "    [Check] Port 53 (UDP/TCP) should show NO traffic (or only blocked attempts)."
  echo "    [Check] Port 853 (TCP) should show active encrypted traffic."
...
# ✅ Terminate tcpdump automatically after 5 seconds or 10 packets.
#    $ sudo timeout 5 tcpdump -ni any port 53 or port 853 -c 10
#    [Check] Port 53 (UDP/TCP) should show NO traffic (or only blocked attempts).
#    [Check] Port 853 (TCP) should show active encrypted traffic.
#
