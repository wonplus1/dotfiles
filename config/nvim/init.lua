...
local function setup_custom_statusline()
  local has_lualine, _ = pcall(require, 'lualine')
  local has_mini_statusline, _ = pcall(require, 'mini.statusline')
  if has_lualine or has_mini_statusline then return end
...
-- {{{ Detects large files and applies performance optimizations automatically.
local function optimize_for_large_files()
  -- ℹ️ Set the file size threshold to 10MB.
  local max_filesize = 10 * 1024 * 1024
  local filepath = vim.fn.expand '%:p'
  local fsize = vim.fn.getfsize(filepath)

  -- 💡 The getfsize function returns -2 for directories or files exceeding system limits.
  if fsize > max_filesize or fsize == -2 then
    -- 🚀 Disable line numbers to reduce rendering overhead.
    vim.opt_local.relativenumber = false
    vim.opt_local.number = false

    -- 🔥 Disable syntax highlighting for maximum performance improvement.
    vim.opt_local.syntax = 'off'

    -- ℹ️ Disable folding and file I/O operations to minimize lag.
