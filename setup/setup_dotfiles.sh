...
symlink_dotfiles() {
  ...
  for item in "${files_to_link[@]}"; do
    local source_file="${dotfiles_base}/${item}"
    local target_file="${HOME}/${item}"

    if [ -e "${source_file}" ]; then
      create_symlink "${source_file}" "${target_file}"
    else
      echo "Skipping: ${item} (Source not found)"
    fi
  done
...
copy_dotfiles() {
  if [ "${is_wsl}" -eq 1 ]; then
    ...
    ideavim_dest="${win_home}/.ideavimrc"

    if [ -f "${vscode_src}" ]; then
      create_dir "$(dirname "${vscode_dest}")"

      ( # Sub Shell
        cd "$(dirname "${vscode_dest}")" || exit
